            ...
            {members.map((member, idx) => (
              <div
                ...
                className={`grid grid-cols-12 gap-2 px-4 py-3 items-center hover:bg-[#1a0b2e]/30 cursor-pointer transition-colors ${
                  idx < members.length - 1 ? "border-b border-[#2a1145]/50" : ""
                }`}
                onClick={() => navigate(`/flexdev/profile/${encodeURIComponent(member.username)}`)}
              >
                <div className="col-span-5 flex items-center gap-3">
                  ...
                  <StyledUsername
                    ...
                    profilePath={`/flexdev/profile/${encodeURIComponent(member.username)}`}
                    className="text-sm"
                  />
